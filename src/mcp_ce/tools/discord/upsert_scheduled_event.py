"""Upsert (create or update) scheduled event in Discord server."""
...
async def upsert_scheduled_event(
    ...
    """
    Create or update a scheduled event in a Discord server.

    If an event with the same name exists, it will be updated.
    Otherwise, a new event will be created.

    This prevents duplicate events from being created during testing.

    Args:
        server_id: Discord server ID
        name: Event name (used to find existing event)
    ...
    Returns:
        ToolResponse with EventResult dataclass and operation type ('created' or 'updated')
    """
    try:
        bot = get_bot()
        ...
        # Add "Last test run" timestamp to description
        current_time = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")

        # If description provided, append timestamp; otherwise just use timestamp
        if description:
            # Remove old "Last test run" line if it exists
            desc_lines = description.split("\n")
            desc_lines = [
                line for line in desc_lines if not line.startswith("Last test run:")
            ]
        ...
        # Build kwargs for create/update
        kwargs = {
            "name": name,
            "start_time": start_dt,
            "entity_type": entity,
        ...
        operation = "updated" if existing_event else "created"

        if existing_event:
            # Update existing event
            await existing_event.edit(**kwargs)
